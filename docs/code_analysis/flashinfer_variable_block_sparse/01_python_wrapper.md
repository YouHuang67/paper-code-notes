...
这是 `plan()` 中最核心的数据变换，由内部函数 `_block_mask_map_to_expanded_indices()` 完成：

```python
def _block_mask_map_to_expanded_indices(
    block_mask_map,  # [H, R, C] bool
    block_col_sz,    # [H, C] int
):
    '''
    将 block 级稀疏邻接图展开为 token 级 CSR 索引。
    输出：
    - kv_indptr: [H*R + 1]，每个 (head, q_block) 对应的 KV token 范围
    - kv_indices: [nnz_tokens]，实际的 KV token 索引
    '''
    # 每个 (head, row) 的 KV token 总数 = 激活的列 block 的 size 之和
    ...
    lengths = block_col_sz[h_idx, c_idx].to(torch.int32)       # 每个激活 block 的大小
    base = head_offset[h_idx] + col_offset[h_idx, c_idx]       # 每个激活 block 的起始 token
...
假设 1 个 KV head，3 个 Q block，4 个 K block：
...
**Q 侧**：`qo_indptr = [0, 2, 5, 6]`

**K 侧索引展开**：

- col_offset = `[0, 3, 4, 6]`（每个 K block 在 8-token 序列中的起始位置）
- head_offset = `[0]`

激活的 (h, r, c) 三元组及展开：

| (h, r, c) | block_col_sz | base | token indices |
|---|---|---|---|
| (0, 0, 2) | 2 | 0+4=4 | 4, 5 |
| (0, 1, 0) | 3 | 0+0=0 | 0, 1, 2 |
| (0, 1, 3) | 2 | 0+6=6 | 6, 7 |
...
- `kv_indptr = [0, 2, 7, 10]`（Q0 看 2 个 token，Q1 看 5 个，Q2 看 3 个）
- `kv_indices = [4, 5, 0, 1, 2, 6, 7, 3, 4, 5]`

这就是 paged attention 看到的输入：对于第 i 个 Q block，它的 KV 范围是 `kv_indices[kv_indptr[i]:kv_indptr[i+1]]`。
