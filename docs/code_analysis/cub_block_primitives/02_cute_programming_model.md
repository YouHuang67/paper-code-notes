...
CuTe（CUTLASS Template）是 NVIDIA CUTLASS 库中的 C++ 模板框架，为 GPU kernel 开发提供**编译时布局推导**和**硬件指令抽象**。它的核心思想是将张量操作分解为 Layout（坐标映射）+ Atom（硬件指令）+ Tiling（线程协作），让开发者在高层描述数据流，由编译器生成最优的 PTX 代码。

本文基于 CUTLASS 源码（随 tilelang 打包的版本）详细介绍 CuTe 的核心概念。所有源码引用均指向本地路径 `$CUTLASS/include/cute/`。
