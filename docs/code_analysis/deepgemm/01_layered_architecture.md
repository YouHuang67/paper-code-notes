...
## 1. impl 按角色切 smem 与 barrier

[`sm100_fp8_fp4_gemm_1d1d.cuh`](https://github.com/deepseek-ai/DeepGEMM/blob/88965b0/deep_gemm/include/deep_gemm/impls/sm100_fp8_fp4_gemm_1d1d.cuh) 把线程分成非 epilogue 与 epilogue 两套 launch bound。shared memory 按 stage 切成 CD store、A tile、B tile、SFA/SFB；`PatternVisitor` 把裸偏移收成「第 \(i\) 个 stage 的视图」。barrier 按职责切：TMA full/empty、带 SF 的 `with_sf_full`、TMEM 交给 epilogue 的 full/empty。摘录见[附录 A](#app-pattern-visitor)。
...
SM90 FP8 1D2D 沿用同一切分：smem 顺序为 D、A stages、B stages、SFA stages、整段 SFB，然后 TMA full/empty barrier。SF 为 FP32。
...
contiguous 且 multicast 落在 A 的对侧时，peer tile 的 expert id 必须相同，否则该次装载退回单 CTA。
...
math 侧同样 `while (scheduler.get_next_block(...))`：等 full barrier、WGMMA、用 `scale_a * scale_b` 提升到 FP32 累加器、empty barrier 放行生产者。TMA 与 MMA 解耦后，同一套 scheduler 可以驱动多阶段流水。SM100 上 TMA / UMMA / epilogue 如何分 warp，见 [02](02_sm90_sm100_pipeline.md)。

## 4. mma 层只构造描述符几何

[`mma::sm90::make_smem_desc`](https://github.com/deepseek-ai/DeepGEMM/blob/88965b0/deep_gemm/include/deep_gemm/mma/sm90.cuh#L196-L210) 把 smem 指针编成 `cute::GmmaDescriptor`（start address、layout type、leading/stride byte offset）。[`mma/sm100.cuh`](https://github.com/deepseek-ai/DeepGEMM/blob/88965b0/deep_gemm/include/deep_gemm/mma/sm100.cuh) 产出 `cute::UMMA::SmemDescriptor`：version、swizzle layout type、base32、MN/K major。
...
layout / scheduler / mma / epilogue 边界固定之后，impl 只协调：哪个 warp 发 TMA、哪个 stage 等哪组 barrier、SF 与主矩阵如何交错、TMEM 或寄存器结果何时交给 epilogue。SM90 1D2D 的交界是 full/empty barrier 与 warpgroup arrive/commit/wait；SM100 1D1D 另加 TMEM full/empty 与独立 epilogue 线程集。下一节按这条交界对比两代流水。

## 附录 A {#app-pattern-visitor}
