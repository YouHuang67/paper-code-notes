...
GEMM 计算 $C_{M \times N} = A_{M \times K} \times B_{K \times N}$（CuTe 中 B 用 shape (N,K) 表示，但数学上等价于 $B^T$ 参与矩阵乘）。K 是归约维度——A 的一行与 B 的一行沿 K 做点积求和，K 不出现在输出 C 中。每个 CTA 负责 C 的一个 128×128 输出块，需要沿归约维度 K 将 A 和 B 切成 64 一段的 tile 逐个加载、乘累加：

| Tensor | Shape | 含义 |
|--------|-------|------|
| mA | (M, K) | 完整 A 矩阵，K-major（K 维度步长为 1，即同一行内沿 K 方向的元素在内存中连续），stride = (ldA, 1) |
| mB | (N, K) | 完整 B 矩阵，K-major，stride = (ldB, 1) |
| mC | (M, N) | 完整 C 矩阵，M-major（M 维度步长为 1，即同一列内沿 M 方向的元素连续），stride = (1, ldC) |
| gA | (128, 64, k) | 当前 CTA 的 A tile，128 行(M) × 64 列(K)，k = ceil(K/64) 个归约 tile |
| gB | (128, 64, k) | 当前 CTA 的 B tile，同上 |
| gC | (128, 128) | 当前 CTA 的 C 输出块 |
...
每个线程持有 MMA 操作数 fragment 和累加器。一个 128×64 的 SMEM tile 沿 K 维度被切成 4 个 16-element 的子块（k_block），线程在内循环中依次将这 4 个子块从 SMEM 加载到寄存器并计算，实现寄存器级流水线：

| Tensor | Shape | 含义 |
|--------|-------|------|
| tCrA | (MMA, MMA_M=4, MMA_K=4) | A 操作数寄存器 fragment |
| tCrB | (MMA, MMA_N=4, MMA_K=4) | B 操作数寄存器 fragment |
| tCrC | (MMA, MMA_M=4, MMA_N=4) | 累加器，跨所有归约 tile 累加 |

- MMA_M=4：CTA 的 128 行输出被 TiledMMA 的 32 行 tile 覆盖，需要 128/32 = 4 次迭代
- MMA_N=4：同理，128/32 = 4 次
- MMA_K=4：一个 SMEM tile 的 64 列(K) 被 MMA 的 16 列消费，需要 64/16 = 4 次内循环
...
- **Retiling 原理**：`tCrA` 和 `tXrA` 指向**同一块寄存器**，但对这些寄存器有不同的逻辑分组方式。MMA 视角按"第几个 atom 的第几个元素"分组 `(mma, mma_m, mma_k)`，ldmatrix 视角按"一次加载指令的第几个元素"分组 `(cpy, mma_m, mma_k)`。`retile_D` 只改变第一维的逻辑解释，不移动任何数据。`make_tiled_copy_A/B` 负责自动计算这个重解释所需的布局变换——它查询 MMA 的线程-值映射，与 ldmatrix 的 Copy_Atom 对齐，生成兼容两者的 TiledCopy
