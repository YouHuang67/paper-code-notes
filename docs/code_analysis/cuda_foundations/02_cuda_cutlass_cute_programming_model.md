...
tags:
  ...
  - CUTLASS
---
# CUDA 基础：CUTLASS/CuTe 编程模型

本文汇总 CUTLASS 的模板分发体系与 CuTe 的 Tensor/Layout/Atom 抽象，作为各类 CUDA kernel 分析的公共引用页。

阅读顺序建议：

- 先看 [CUDA 基础：执行模型与内存访问](01_cuda_execution_model_and_memory.md)，再回到本文看 CUTLASS / CuTe 的模板与布局抽象
...
本文只保留能在本地 CUTLASS / CuTe 头文件中直接落到类型定义、模板特化或内联 PTX 的内容。主要依据包括：

- CUTLASS：`cutlass/gemm_coord.h`、`cutlass/arch/arch.h`、`cutlass/arch/mma.h`、`cutlass/gemm/device/default_gemm_configuration.h`
- CuTe：`cute/tensor_impl.hpp`、`cute/layout.hpp`、`cute/arch/mma_sm80.hpp`、`cute/atom/mma_traits_sm80.hpp`、`cute/atom/mma_atom.hpp`、`cute/arch/copy_sm80.hpp`、`cute/arch/copy_sm75.hpp`、`cute/atom/copy_atom.hpp`、`cute/swizzle.hpp`

如果某个性能结论或编译器行为没有在这些本地文件中直接体现，本文只给出结构性解释，不把它写成硬结论。
...
**编译时常量**：CuTe 使用 `Int<N>` (即 `cute::constant<int, N>`) 表示编译时整数。例如 `_64{}` 是 `Int<64>` 的实例。当 Shape 和 Stride 全部由 `Int<N>` 构成时，Layout 中的偏移表达式也全部由编译期常量构成，运行时无需再读取动态 shape / stride。
...
Stride 将 `(thread_id, value_id)` 映射到矩阵中的线性偏移。这些布局本身以类型和静态常量的形式存在，不需要额外的运行时描述符。
...
这就是为什么 CuTe 代码中常常看不到手动写 PTX 名称：地址空间标记和 atom 类型会共同决定编译期选择哪条搬运路径。
...
这需要两倍的 smem 缓冲区（KV 各两份）。它的目的是让数据加载与计算重叠；是否值得、收益多大，要看具体 kernel 的寄存器和 smem 预算。
...
结果：`sVt(d, n)` 访问的物理地址等于 `sV(n, d)` 的地址——**不移动任何数据**，只是改变索引方式。配合 `ldmatrix.trans`，转置效果发生在“smem 视图 + 加载路径”这一层，而不是先做一轮显式搬运。
...
- **ldmatrix + ldmatrix.trans**：smem→register 加载，寄存器分布直接匹配 mma.sync 要求。V 的转置由 composition + ldmatrix.trans 在加载阶段完成，不需要额外单独搬运
