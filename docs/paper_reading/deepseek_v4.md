...
不过“保留 MoE 主干”不等于“MoE 完全没变”。论文在这部分仍然做了几处重要微调：

- affinity score 的激活函数从 `Sigmoid` 改成 `Sqrt(Softplus)`，说明作者在 router 的数值形态上做了重新权衡
- 负载均衡仍沿用 auxiliary-loss-free 策略，但额外加入了轻量的 sequence-wise balance loss，用来避免单条序列内部极端失衡
- 移除了 routing target node 数量限制，并相应重做并行策略，以避免新架构下被旧并行假设反向限制
- 最前面的若干 Transformer blocks 不再先走 dense FFN，而是直接采用 **Hash routing 的 MoE 层**；这相当于在浅层用更保守、更规则化的专家分流方式，降低 learned routing 的不稳定性

Multi-Token Prediction (MTP) 则基本保持不变。这一点很关键：它说明论文真正想验证的新变量，确实集中在 residual / attention / optimizer / system 这些部分，而不是靠同时大改所有模块去“堆”结果。
...
**第二步：Lightning Indexer 在压缩块空间里做检索**
...
**attention sink：允许 head 暂时“不看”真实记忆**
...
<span id="appendix-a-symbols"></span>
**附录 A：主要符号表**
...
<span id="appendix-b-configs"></span>
**附录 B：模型配置与参数规模**
...
<span id="appendix-c-training"></span>
**附录 C：预训练与稳定性细节**
...
<span id="appendix-d-systems"></span>
**附录 D：系统与基础设施细节**
...
论文这里还有一个非常值得记住的硬件结论：**能否彻底隐藏通信，不主要看带宽绝对值，而看 compute / bandwidth 是否与每 token-expert 的算通比匹配。** 对 DeepSeek-V4-Pro，作者把条件化简为

$$
\frac{C}{B} \le 2d = 6144 \text{ FLOPs/Byte}
$$

也就是说，每 1 GBps 互连带宽大约足以隐藏 6.1 TFLOP/s 计算。论文借此提出一个很明确的硬件共设计观点：带宽并非越多越好，一旦越过这个平衡点，再继续堆带宽的边际收益会迅速下降。

另外他们还给了三条面向硬件厂商的建议：

- 为极端 kernel fusion 预留更充足的 power budget，避免 compute / memory / network 同时满载时被功耗墙卡住
- 提供更低延迟的跨 GPU signaling，使 fine-grained push 式通信成为可能
- 探索比 SwiGLU 更便宜的 element-wise activation，以进一步降低 post-GEMM 代价
...
更细一点看，TileLang 在论文中承担了三层职责：

- **快速原型职责**：注意力变体、压缩算子、验证性 kernel 可以快速落地
- **编译优化职责**：借助 Z3 的整数分析，支持 layout inference、vectorization、barrier insertion 等更复杂 pass
- **一致性职责**：尽量和手写 CUDA baseline 在代数化简与 lowering 顺序上保持一致，帮助实现 bitwise 对齐

论文给出的一个很硬的数字是：Host Codegen 能把 CPU 侧 validation / argument marshaling 开销从“几十到上百微秒”降到“每次调用不到 1 微秒”。这说明在极高性能 kernel 上，host overhead 本身已经成为必须显式优化的一部分。
...
这里最值得注意的一点是：V4 把“训练复现性”当成基础设施目标，而不是 debugging 时顺手加一下。因为一旦模型训练中出现 loss spike、数值异常或 kernel bug，determinism 能显著缩小排查空间，也让 post-training / inference 与 pre-training 的 bitwise 对齐更可验证。
...
论文这里还有一个非常关键但容易被忽略的点：他们强调 **FP4 → FP8 的反量化在当前条件下是“无损”的**。原因是 FP8(E4M3) 比 FP4(E2M1) 多 2 个指数位，只要 FP4 子块之间的 scale ratio 不超过阈值，子块 scale 信息可以被 FP8 的额外动态范围吸收。因此整个 QAT 流程能复用既有的 FP8 训练框架，而不必重写一条全新的低精度训练栈。
...
- 为避免低精度加法器的累积误差，不采用常规 tree/ring reduce-scatter，而是先 all-to-all 再本地 FP32 求和
...
这一节还有一个常被忽视的补充点：论文额外实现了 **tensor-level activation checkpointing**。它不是传统“整层 checkpoint/recompute”，而是让开发者在 forward 中标注具体 tensor，再由框架自动追踪最小重算子图。这样既保留自动求导框架的便利性，又把重算粒度细化到 tensor 级别，对 V4 这种大量 fused op 和复杂中间态的模型非常重要。
...
论文还更具体地说明了 cache block 的组织思想：

- state cache 为每个 request 预分配固定大小 block，用来放 SWA 的最近窗口状态以及尚未凑满压缩块的尾 token 状态
- classical KV cache 的 block 大小则按 $\mathrm{lcm}(m,m')$ 的原始 token 数设计，以同时兼容 CSA 与 HCA 的压缩对齐要求

这实际上说明：**V4 的 KV cache layout 已经是模型定义的一部分**，而不只是一个部署工程细节。
...
对于 Zero SWA Caching，论文给出的关键洞见是：每层某个 token 的 SWA KV 只依赖上一层最近 $n_{win}$ 个 token 的 SWA 状态。因此，要恢复一个 $L$ 层模型的最后窗口状态，并不需要重算整个前缀，只需要重算最后 $n_{win}\cdot L$ 个 token。这个依赖结构正是该策略可行的根本原因。

<span id="appendix-e-posttraining"></span>
**附录 E：后训练与 agent 化能力**
...
更具体地说，三种模式对应不同的 response format 与评测上下文预算：

- Non-think：更像快速响应模式，输出更短、上下文预算更小
- Think High：要求显式输出 `<think> ... </think>` 推理过程
- Think Max：除了 `<think>` 格式外，还在 system prompt 中注入“absolute maximum, no shortcuts”这类强约束，刻意把模型推到 reasoning 上限

在标准评测里，论文给出的三个 context window 预算分别是：
...
- `<|authority|>`：判断权威性需求
- `<|domain|>`：识别问题领域
- `<|extracted_url|>` / `<|read_url|>`：判断用户给出的 URL 是否需要抓取与阅读

这些任务可直接复用已有 KV cache，避免额外 prefill。

论文这里真正的系统意图是：把一部分“本来要交给辅助小模型”的工作，重新收回主模型，通过 special token 触发并复用同一份 KV cache。这样减少了工程复杂度，也缩短了 TTFT（time-to-first-token）。
...
- **Teacher scheduling**：因为 full-vocabulary OPD 面对的是超大 teacher 集合，论文只缓存最后一层 hidden states，而不是直接缓存 logits；并通过按 teacher index 对样本排序，保证同一 mini-batch 中最多只有一个 teacher head 常驻显存
- **Token-granular WAL**：如果 rollout 中断后简单从头重生成，会产生长度偏置，因为短答案更容易在中断前完成；WAL 的意义不仅是容错，也是保证 RL/OPD 训练目标的统计正确性

至于 DSec，本质上是一个面向 agentic AI 的大规模 sandbox 平台。论文把它拆成四类 execution substrate：Function Call、Container、microVM、fullVM，并强调 layered image loading、trajectory logging 与 preemption-safe resumption 这三件事是支撑大规模 agent 训练的重要基础设施。

<span id="appendix-f-eval"></span>
**附录 F：评测补充**

**F.0 评测设置与口径**

这部分虽然不直接贡献模型设计，但对正确理解结果非常重要。论文在评测上做了几件值得注意的事：

- **Codeforces 不是简单 pass@k**：作者构造了 14 场 Division 1 contests、114 道题，对每题生成 32 个候选解，再随机无放回采样 10 个构成提交序列，用专家测试集判定，并按 OpenAI 方案映射到 contest rating，最后再对随机顺序求期望 Elo
- **数学评测区分普通推理与形式化证明**：除 HMMT、Apex 等普通题外，作者还在 Lean v4.28.0-rc1 上评估 formal math，最多允许 500 次工具调用
- **1M context 评测专门重跑竞争对手**：MRCR 和 CorpusQA 上，论文重新评估了 Claude Opus 4.6 与 Gemini 3.1 Pro，试图统一配置口径
- **Agent 评测限制清晰**：内部 code-agent harness 只给 bash 工具和 file-edit 工具，最多 500 步、最大 512K context；search-agent 则给 websearch 与 Python 工具

因此，V4 的许多结果并不是“API 直接调用一把出分”，而是有一套明确的评测协议支撑。
...
论文在这部分还有两个非常亮眼的结果值得单独记下：

- **形式化数学**：在轻量 agent 设定下，DeepSeek-V4-Flash-Max 的 Putnam-200 Pass@8 达到 81.0；在 frontier 设定下，DeepSeek-V4 达到 Putnam-2025 的 120/120 proof-perfect
- **reasoning token efficiency**：Figure 10 说明，V4 不仅在总推理预算上更强，而且在 HLE 这类任务上，单位 token 的 reasoning 效率也优于前代 V3.2
...
- **中文功能写作**：V4-Pro 对 Gemini-3.1-Pro 的总体胜率为 62.7% vs 34.1%
- **中文创意写作**：instruction following 胜率 60.0%，writing quality 胜率 77.5%
- **高复杂约束写作**：Claude Opus 4.5 仍略占优，52.0% vs 45.9%
- **白领任务**：V4-Pro-Max 对 Opus-4.6-Max 的 overall non-loss rate 达 63%，强项是 Task Completion 与 Content Quality，弱项是部分格式约束跟随与视觉排版
- **内部 R&D coding benchmark**：V4-Pro-Max pass rate 67%，明显高于 Sonnet 4.5 的 47%，接近 Opus 4.5 的 70%

这些结果说明，V4 的收益非常集中地体现在“长链任务 + 长上下文 + 工具交互 + 高复杂度中文场景”这些传统 benchmark 难以完整覆盖的地方。

**F.5 结论、局限与未来方向**

论文最后的 conclusion 也值得记进笔记，因为它说明作者如何看待 V4 自己的定位。

作者给出的核心自我判断是：

- V4 已经把 open long-context model 推进到了新的效率-能力边界
- 但为了降低风险，架构里保留了很多“先验上已验证有效的小技巧”，导致整体设计仍偏复杂

作者明确承认的局限与后续方向包括：

- **架构复杂度仍偏高**：未来会做更系统、更原则化的化简，提炼真正必要的核心设计
- **训练稳定性原理尚未吃透**：Anticipatory Routing 与 SwiGLU Clamping 虽有效，但理论解释不足
- **稀疏化维度还会继续扩展**：不止 MoE 与 sparse attention，还会探索更 sparse 的 embedding 等新维度
- **继续优化低时延与长链 agent 场景**：让长上下文部署更灵敏、更可交互
- **继续走向多模态与在线学习范式**：论文明确提到未来会继续补充 multimodal capability，并探索 online learning 一类新范式

<span id="appendix-g-code-roadmap"></span>
**附录 G：后续代码解析路线图**
