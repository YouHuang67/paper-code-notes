...
<a id="sec-2-overview"></a>
## 2. 总体架构：把 Transformer 改造成分层压缩记忆系统
...
<a id="sec-3-mhc"></a>
## 3. mHC：把 residual 从单通道加法，改造成受约束的多通道混合
...
<a id="sec-4-hybrid-attn"></a>
## 4. 混合注意力：把长上下文改写为压缩记忆检索
...
<a id="sec-6-synthesis"></a>
## 6. 把各模块拼起来看：DeepSeek-V4 到底是怎样工作的
...
<a id="sec-7-results"></a>
## 7. 核心结果：V4 证明这套记忆系统确实值回票价
...
<a id="appendix-e-posttraining"></a>
## 附录 E：后训练与 agent 化能力
...
<a id="appendix-g-code-roadmap"></a>
## 附录 G：后续代码解析路线图
...
- **本文真正的主角不是 sparse attention，而是 memory representation。**
- **CSA / HCA / SWA 是三种不同粒度的记忆访问机制，而不是三个并列的小技巧。**
- **mHC 的意义不在 long-context 本身，而在让这种复杂记忆系统可稳定堆叠。**
- **Muon 的意义不在替代 AdamW 本身，而在让大矩阵子系统的联合训练更稳定。**
- **V4 的论文价值，很大一部分来自它把“模型设计”和“系统兑现”写成了一条连续主线。**
