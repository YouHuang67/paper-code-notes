...
Flow Matching [Lipman et al., 2023; Liu et al., 2023; Albergo & Vanden-Eijnden, 2023] 定义了一条从噪声到数据的连续流路径。设  \sim p_{data}(x)$ 为数据分布，$\epsilon \sim p_{noise}(\epsilon)$ 为噪声分布（标准高斯）。通过**线性插值（rectified flow）**定义含噪隐变量：

2419106z_t = t x + (1 - t) \epsilon, \quad t \in [0, 1]2419106

边界条件： = \epsilon$（纯噪声）， = x$（干净数据）。

在连续时间中，**流速 (velocity)** 定义为 $ 对时间 $ 的导数：

2419106v(z_t, t) = rac{dz_t}{dt} = x - \epsilon2419106

Flow Matching 的目标是训练神经网络 	heta(z_t, t)$ 来拟合真实流速 $，损失函数为：

2419106\mathcal{L}_{FM} = \mathbb{E}_{t, x, \epsilon} \left\| v_	heta(z_t, t) - v ight\|^22419106

#### x-prediction 参数化

ELF 不直接预测 $，而是预测干净嵌入 $（x-prediction），原因有二：
...
2419106v(z_t, t) = rac{x - z_t}{1 - t}2419106

代入 Flow Matching 损失即可得到 x-prediction 形式的 MSE 损失：

2419106\mathcal{L}_{MSE} = \mathbb{E}_{t, x, \epsilon} \left\| v_	heta(z_t, t) - v ight\|^2 = \mathbb{E}_{t, x, \epsilon} rac{1}{(1 - t)^2} \left\| x_	heta(z_t, t) - x ight\|^22419106

这一形式的重要性在于：当  	o 1$ 时，^{-2}$ 使损失权重趋向无穷大，自然强调接近干净数据时的精细预测。
...
在送入 ELF 网络前，嵌入经过 bottleneck 线性投影降维到 128 维（降维有利于模型学习低维流形上的去噪，详见后文消融），再映射回模型 hidden size 768。

**嵌入归一化**：训练前先在整个数据集上估计嵌入的均值和标准差，用这些统计量对干净嵌入做通道级归一化。这一步对稳定训练很重要。

### 连续嵌入上的 Flow Matching

获得 $x$ 后，定义去噪过程。采样时间步 $t$（从 logit-normal 分布采样，$P_{mean} = -1.5, P_{std} = 0.8$），构造含噪嵌入：

$$z_t = t \cdot x + (1 - t) \cdot \text{scale} \cdot \epsilon, \quad \epsilon \sim \mathcal{N}(0, I)$$

其中噪声缩放因子 $\text{scale} = 2.0$（denoising branch）。

网络 $x_\theta(z_t, t, \text{mode})$ 预测干净嵌入 $\hat{x}$，训练目标为 MSE：

$$\mathcal{L}_{MSE} = \mathbb{E}_{t, x, \epsilon} \frac{1}{(1 - t)^2} \| x_\theta(z_t, t, \text{"denoise"}) - x \|^2$$

**时间步采样**：$t$ 从 logit-normal 分布采样，即 $t' \sim \mathcal{N}(P_{mean}, P_{std}^2)$，$t = \sigma(t')$（$\sigma$ 为 sigmoid）。这使采样集中在中间区域，两个极端（$t \approx 0$ 和 $t \approx 1$）采样较少但不为零。

$$
t \sim \sigma\big(\mathcal{N}(P_{mean}, P_{std}^2)\big)
$$
...
- 在训练时，除了 denoising branch（MSE loss），还有 decoding branch（CE loss）
...
其中 $p$ 是 per-token 的 corruption level，从另一个 logit-normal 分布采样（$P_{mean} = 0.8, P_{std} = 0.8$），$\text{scale}_{dec} = 5.0$（OWT 上）。

每个 token 的 $p$ 独立采样，意味着同一序列中某些 token 几乎干净、某些高度噪声。这迫使 decoder-mode 网络从上下文中恢复被破坏 token——恰好模拟了推理时 denoiser 产生的不完美嵌入。
...
训练时 80% 的 batch 分配为 denoising mode（MSE），20% 为 decoding mode（CE）。这是重要的超参——实验表明更高比例的 denoising mode 有利于质量-多样性 trade-off。
...
if uniform(0, 1) < 0.8:           # denoising branch (80%)
    ...
    p = sample_p()                  # per-token logit-normal 采样
...
推理从纯高斯噪声 $z_0 \sim \mathcal{N}(0, I)$ 开始，沿 ODE $dz_t/dt = v_\theta(z_t, t)$ 逐步去噪。时间区间 $[0, 1]$ 离散化为 $T$ 个区间（默认用 logit-normal schedule）。
...
**SDE 采样**（随机性）：
SDE variant 在每一步后重新注入少量高斯噪声，同时将时间变量 $t$ 向噪声方向回退。这模拟了 Flow Matching 底层 SDE 的随机行为（参考 SiT [Ma et al., 2024]）。

参数 $\gamma$ 控制噪声重注入量：
- $\gamma = 0$：退化为 ODE
- $\gamma = 1.0$：默认设置，在质量-多样性 trade-off 上最佳
- 更大 $\gamma$：更多随机性，倾向于更低 Gen. PPL 但稍低熵
...
**时间调度 (time schedule)**：推理时使用 logit-normal 时间调度——在 $t$ 接近 0 时分配更细的时间步，$t$ 接近 1 时步长更大。这因为噪声大的区域需要更精细的离散化，且与训练时的 logit-normal 分布匹配。实验证实该 schedule 在所有步数下都优于 uniform schedule，在少步时尤其明显。
...
实现方式：将 $z_t$ 与 $\hat{x}_{i-1}$ 沿通道维度拼接（channel 维度翻倍），再通过一个线性层投影回原始维度。

训练时以 50% 概率使用 self-conditioning（拼接前一步预测），50% 概率使用零向量（学习无条件路径）。推理时以上一步预测作为条件，**不增加额外 forward pass**。
...
$$
u^{target} = v + \left(1 - \frac{1}{\omega}\right) \left(v_\theta(z_t | c, \omega) - v_\theta(z_t | \emptyset, \omega)\right)$$

其中 $\omega$ 是 guidance scale。当 $\omega = 1$，$\nu^{target} = v$（退化为无 CFG）。
...
- **评估**：生成 1000 样本，以 GPT-2 Large 计算 Gen. PPL + unigram entropy（质量-多样性 trade-off）
...
- **训练**：Muon optimizer, lr=0.002, batch=512. OWT 5 epochs (~95K steps), WMT14/XSum 100 epochs. TPU v5p × 64, ~1.5h/epoch
- **推理**：默认 SDE sampler + logit-normal time schedule，self-conditioning CFG scale 可在推理时灵活调整

### 消融实验关键发现

**1. CFG scale**（Fig. 4）：
- 增大 CFG scale → Gen. PPL 下降，熵也下降（质量-多样性 trade-off）
- 最优方向是图的右下角（低 Gen. PPL + 高熵）

**2. 嵌入选择**（Fig. 5a）：
- 上下文嵌入（T5 encoder）> 非上下文嵌入（单层 embedding）
- 预训练 encoder > 从零训练的 encoder > 可学习 embedding（最差）
- 高斯初始化冻结 embedding 也有一定效果（作为强 baseline 很意外）

**3. 解码策略**（Fig. 5b）：
- 共享权重 vs 两阶段独立 decoder：两者 trade-off 相近
- 但共享权重在低 Gen. PPL 区域延伸更远，且 pipeline 更简洁（无额外训练阶段）

**4. 采样器**（Fig. 5c）：
- SDE 在少步时显著优于 ODE
- 随步数增加差距缩小，但 SDE 始终更好
- 8 步 SDE 约等于 64 步 ODE 的质量

**5. x/v/ε-prediction**（Appendix Fig. 10）：
- x-prediction 在 512/768/1024 维度都稳定
- v-prediction 在 512-d 可接受，768/1024-d 退化
- ε-prediction 在所有维度崩塌（极高 Gen. PPL 或极低熵）

**6. Bottleneck 维度**（Fig. 11）：
- 128-d 最佳平衡；32-d 低熵（多样性差），512-d 高 Gen. PPL（质量差）
- Bottleneck 的存在本身很重要：压缩到低维帮助模型在流形上学习

**7. Denoising mode probability**（Fig. 12）：
- 0.8（80% MSE, 20% CE）最优
- 过低会导致去噪过程训练不足

**8. 条件策略**（Fig. 13）：
- In-context conditioning（prepend 条件 token）略优于 adaLN-Zero
- 同时大幅减少参数（148M → 105M）

**9. 优化器**（Fig. 14）：
- Muon > AdamW，但两者训练的模型都超越 baseline
- 说明 ELF 的优势不能仅归因于优化器

**10. 时间调度**（Fig. 15a）：
- Logit-normal 在所有步数都优于 uniform，少步时优势明显
- 因为它使推理时的 t 分布匹配训练时

**11. SDE γ**（Fig. 15b）：
- 在适度范围内增大 γ 降低 Gen. PPL 但稍降熵
- γ = 1.0 提供最佳平衡
...
ELF-B 用 32 步达到比蒸馏模型更好的质量，训练 token 仅为其 1/12。

在极少步（8-16 步）时，ELF 略弱于最佳蒸馏模型，但仍远超非蒸馏 baseline。
...
- ELF-B (105M) → ELF-M (342M) → ELF-L (652M)：模型增大，Gen. PPL-熵 frontier 一致向右下移动（更好）
- ELF-L 在 CFG=3 时 Gen. PPL = 23.3，熵 = 5.28（64 步 SDE）
...
ELF 的完整训练 pipeline 如下（对应论文 Fig. 9 + Alg. 3, 4）：
...
- **Control tokens**：4 time tokens（值 ∈ [0,1]）+ 4 CFG tokens（值 ∈ [0.5, 5]）+ 4 mode tokens（denoise/decode）。token embedding 维度与模型 hidden size 一致，与数据序列 concat 后共同做 self-attention
- **Training-time CFG**：对每个 sample，网络做两次 forward：unconditional（$\hat{x}_{no\\_sc}$）和 conditional（$\hat{x}_{sc}$），然后按 CFG 公式组合 velocity target。利用 in-context CFG scale token 让网络学习 $\omega$ 相关的 velocity field
