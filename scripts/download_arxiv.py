...
Accepts an arXiv URL (abstract, PDF, or bare ID), downloads the PDF to
refs/papers/, then invokes split_pdf.py to split and extract text.

Usage:
    python scripts/download_arxiv.py https://arxiv.org/abs/1706.03762
    python scripts/download_arxiv.py https://arxiv.org/pdf/2504.13074v3
    python scripts/download_arxiv.py 2504.13074
"""
...
def download_pdf(arxiv_id: str, save_dir: Path) -> Path:
    ...
    data = resp.read()
    if len(data) < 1024:
        print("Download failed: response too small, check the arXiv ID")
        sys.exit(1)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python scripts/download_arxiv.py <arxiv_url_or_id>")
        sys.exit(1)
