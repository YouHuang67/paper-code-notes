...
- **禁止用 Agent 子代理读取论文**：直接在主对话中逐一读取和处理，不启动并行 agent

### 笔记规范
- **开头**：论文链接 + 代码链接 + 团队（主要机构）。然后是「概述」章节，具体描述论文解决什么问题、怎么解决的、整体流程和主要结论，读完概述即可掌握全貌
- **正文**：按论文顺序展开，技术细节不省略（数学公式、训练配置、数据规模等），但行文简洁不啰嗦
- **格式**：纯文本 + 分点罗列为主，表格仅用于实验结果对比，不用 admonition 等花哨语法
- **末尾**：关键启示，提炼可迁移的设计思路

### 团队/机构提取规则
- 笔记开头需包含 `**团队**: xxx` 或 `- 团队：xxx` 格式
- 机构信息提取自论文一二作和尾作单位，根据具体情况简化显示
- 格式示例：
  - 单一机构：`Skywork AI`、`Qwen Team, Alibaba Group`
  - 多机构合作：`CUHK MMLab, Tsinghua University, Shanghai AI Lab`
- 机构信息会被提取到 `tag_index.json` 的 `organizations` 字段
